...
void rf05_Combined()
{
   ...
   // TFile *f_FS = new TFile("Ds2D0enue_Analysis/05-ROOT/Fit_Parameter/rf03_FakeSignalPara.root");
   // TFile *f_O = new TFile("Ds2D0enue_Analysis/05-ROOT/Fit_Parameter/rf04_OtherPara.root");
   ...
   // // Fake Signal
   // RooWorkspace *w_FS = (RooWorkspace *)f_FS->Get("w_FS");
   // // RooRealVar *M_CFBkg = w_CFBkg->var("Ds_diff_D0pi");
   // RooAbsPdf *FakeSignal = w_FS->pdf("FakeSignalModel");

   // // Other
   // RooWorkspace *w_O = (RooWorkspace *)f_O->Get("w_O");
   // // RooRealVar *M_OBkg = w_CFBkg->var("Ds_diff_D0pi");
   // RooAbsPdf *Other = w_FS->pdf("OtherModel");
   ...
   // w_FS->loadSnapshot("reference_fit_FakeSignal");
   ...
   RooDataSet data("data", "dataset with mass", tree, RooArgSet(DeltaM,Veto,BCS,dM));
   //=========================================================================================================================================================================//
   ...
   //--------------------------------------------------------------------------//
   RooRealVar f0("f0", "f0", 0, -10., 10.);
   RooRealVar f1("f1", "f1", 52560, 40000, 60000);
   RooRealVar f2("f2", "f2", 482221, 200000, 600000);
   RooRealVar f3("f3", "f3", 20181, 10000., 25000.);
   RooRealVar f4("f4", "f4", 3.0000e+04, 5000., 30000.);
   // f0.setConstant(true);
   // f1.setConstant(true);
   // f2.setConstant(true);
   // f3.setConstant(true);
   ...
   // RooAddPdf model("model", "s+pbkg+cbkg+fs", RooArgList(*SignalModel,*PeakingBkgModel,*Background,*FakeSignal,*Other), RooArgList(f0,f1,f2,f3,f4));
   //=========================================================================================================================================================================//
   ...
   std::unique_ptr<RooFitResult> r{model.fitTo(data, Save(), PrintLevel(1), Range(0.13957,xR))};
   ...
   model.plotOn(frame, Components(CFBkgModel),      LineColor(kRed),                     RooFit::Name("Background"));
   model.plotOn(frame, Components(FakeSignalModel), LineStyle(kDashed), LineColor(9),    RooFit::Name("FSignal"));
   model.plotOn(frame, Components(OtherModel),      LineStyle(kDashed), LineColor(42),   RooFit::Name("OtherF"));
   ...
   N_FS.Form("N_{FS} = %.2f #pm %.2f", f3.getValV(), f3.getError());
   ...
   legend->AddEntry((TObject *)0, N_FS, "");
   legend->AddEntry("OtherF", "Other", "pl" );
   ...
   //-------------------------------
   c->SaveAs("Ds2D0enue_Analysis/09-Images/rf05_DeltaM_Combined.png");
   //=========================================================================================================================================================================//
