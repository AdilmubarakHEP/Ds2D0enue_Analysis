...
void rf01_PeakingBackground()
{
   ...
   // I m p o r t s   D a t a   f r o m   t r e e   b r a n c h:
   // --------------------------------------------------------------
   TFile *file = new TFile("/home/belle2/amubarak/C01-Simulated_Events/Dstarplus-Background.root");
   ...
   // RooRealVar BS("Ds_extraInfo_BkgBDT", "Background Suppression", 0.531, 1);

   // RooDataSet data("data", "dataset with mass", tree, RooArgSet(M,x,y));
   RooDataSet data_PBkg("data_PBkg", "dataset with mass", tree, RooArgSet(DeltaM));
   //=========================================================================================================================================================================//
   // B u i l d   M o d e l
   // -------------------------------------------------------------- 
   // // First Attempt
   ...
   RooRealVar sig_mean("sig_mean", "mean of crystal", 1.4543e-01, 0.145, 0.146);
   RooRealVar sig_sigmaL("sig_sigmaL", "sig_sigmaL", 3.1449e-04, 1e-4, 0.02);
   RooRealVar sig_sigmaR("sig_sigmaR", "sig_sigmaR", 3.4047e-04, 1e-4, 0.02);
   RooRealVar sig_alphaL("sig_alphaL", "sig_alphaL", 1.1515e+00, 1e-4, 4.);
   RooRealVar sig_alphaR("sig_alphaR", "sig_alphaR", 1.1797e+00, 1e-4, 4.);
   RooRealVar sig_nL("sig_nL", "sig_nL", 5.9842e+00, 1e-4, 8);
   RooRealVar sig_nR("sig_nR", "sig_nR", 5.3426e+00, 1e-4, 6);
    sig_mean.setConstant();
    sig_sigmaL.setConstant();
    sig_sigmaR.setConstant();
    sig_alphaL.setConstant();
   ...
   // Combinatorial Part
   // RooRealVar B0_PBkg("B0_PBkg", "B0_PBkg", -40., 20.);
   // RooRealVar P0_PBkg("P0_PBkg", "P0_PBkg", 0., 5.);
   // RooGenericPdf comb_PBkg("comb_PBkg","Generic PDF","(Ds_diff_D0pi - 0.13953 > 0 ? ( ((Ds_diff_D0pi-0.13953)^P0_PBkg)*exp( B0_PBkg*(Ds_diff_D0pi-0.13953))) : 0.0)", RooArgSet( DeltaM, B0_PBkg, P0_PBkg));
   // Constant Background:
   RooRealVar c0("c0", "coefficient #0", 1);
   RooPolynomial comb_PBkg("comb_PBkg", "background p.d.f.", DeltaM, c0);

   // C o n s t r u c t   a   S i g na l   a n d   B a c k g r o u n d   P D F:
   // --------------------------------------------------------------
   RooRealVar Nsig_PBkg("nsig_PBkg", "#Signal events", 1.3979e+05, 0., 500000);
   RooRealVar Nbkg_PBkg("nbkg_PBkg", "#background events", 1.2405e+04, 0., 500000);
   // RooAddPdf model("model", "g+a", RooArgList(PeakingBkgModel), RooArgList(Nsig_PBkg));
   RooAddPdf model("model", "g+a", RooArgList(PeakingBkgModel, comb_PBkg), RooArgList(Nsig_PBkg, Nbkg_PBkg));
   //=========================================================================================================================================================================//
   ...
   std::unique_ptr<RooFitResult> r{model.fitTo(data_PBkg, Save(), PrintLevel(-1))};
   ...
   data_PBkg.plotOn(frame, Binning(bin));
   model.plotOn(frame, Components(comb_PBkg),       LineStyle(kDashed), LineColor(46), RooFit::Name("Background"));
   model.plotOn(frame, Components(PeakingBkgModel), LineStyle(kDashed), LineColor(28), RooFit::Name("Signal"));
   ...
   Int_t npar = model.getParameters(data_PBkg)->selectByAttrib("Constant",kFALSE)->getSize(); //select floating parameters and count their number
   ...
   Nsig_para.Form("N_{sig} = %.2f #pm %.2f", Nsig_PBkg.getValV(), Nsig_PBkg.getError());
   Nbkg_para.Form("N_{bkg} = %.2f #pm %.2f", Nbkg_PBkg.getValV(), Nbkg_PBkg.getError());
   ...
   w->writeToFile("/home/belle2/amubarak/Ds2D0enue_Analysis/05-ROOT/Roofit_Original/Fit_Parameter/rf01_PeakingBkgPara.root");
   //=========================================================================================================================================================================//
   c->SaveAs("/home/belle2/amubarak/Ds2D0enue_Analysis/05-ROOT/Roofit_Original/Fit_Image/rf01_DeltaM_PeakingBkg.png");
