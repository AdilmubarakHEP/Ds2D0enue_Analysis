...
void rf04_Other()
{
   ...
   // I m p o r t s   D a t a   f r o m   t r e e   b r a n c h:
   // --------------------------------------------------------------
   // TFile *file = new TFile("C03-Grid/Completed/Ds2D0e-Generic_Ds_100824_0_ccbar_Dstar+.root");
   ...
   RooRealVar DOther("Ds_Other","Ds_Other",0.5,1.1);

   RooDataSet data_O("data_O", "dataset with mass", tree, RooArgSet(DeltaM,Veto,BCS,dM,DOther));
   ...
   // RooRealVar x00_O("x00_O","x00_O", 0.13957, xL, 0.2);
   // RooRealVar a0_O("a0_O","a0_O",  4.3203e+00, 0.0, 50.);
   // RooRealVar b0_O("b0_O","b0_O", 1.4302e+01, 0.0, 100.);    
   // x00_O.setConstant(true);
   // a0_O.setConstant(true);
   // b0_O.setConstant(true);
   // RooFormulaVar x00prime_O("#x00prime_O", "x[0] < x[1] ? x[0] : x[1]", RooArgList(DeltaM, x00_O));     
   // RooFormulaVar gamma0_O("#gamma0_O", "a0_O + 1", RooArgList(a0_O));    
   // RooFormulaVar beta0_for_gamma_O("beta0_for_gamma_O", "1./b0_O", RooArgList(b0_O));    
   // RooGamma GammaModel1_O("GammaModel1_O", "Gamma pdf", DeltaM, gamma0_O, beta0_for_gamma_O, x00prime_O);

   // // Second Gamma Function
   // RooRealVar x01_O("x01_O","x01_O", 0.13957, xL, 0.2);
   // RooRealVar a1_O("a1_O","a1_O", 7.9266e-01, 0.0, 50.);
   // RooRealVar b1_O("b1_O","b1_O", 1.7561e+01, 0.0, 200.);     
   // x01_O.setConstant(true);
   // a1_O.setConstant(true);
   // b1_O.setConstant(true);
   // RooFormulaVar x01prime_O("#x01prime_O", "x[0] < x[1] ? x[0] : x[1]", RooArgList(DeltaM, x01_O));     
   // RooFormulaVar gamma1_O("#gamma1_O", "a1_O + 1", RooArgList(a1_O));    
   // RooFormulaVar beta1_for_gamma_O("beta1_for_gamma_O", "1./b1_O", RooArgList(b1_O));    
   // RooGamma GammaModel2_O("GammaModel2_O", "Gamma pdf", DeltaM, gamma1_O, beta1_for_gamma_O, x01prime_O);

   // // Add the components
   // RooRealVar f0_O("f0_O", "f0", 3.9786e-01, 0, 1);
   // RooRealVar f1_O("f1_O", "f1", 1.3991e-01, 0, 1);
   // f0_O.setConstant(true);
   // f1_O.setConstant(true);
	// RooAddPdf OtherModel("OtherModel","g1+g2",RooArgList(GammaModel1_O,GammaModel2_O), RooArgList(f0_O,f1_O));

   // F i n a l    S i g n a l   C o u n t
   //-----------------------------------------
   RooRealVar Nsig_O("Nsig", "#Signal events", 0., 20000);
   ...
   std::unique_ptr<RooFitResult> r{model.fitTo(data_O, Save(), PrintLevel(-1), Range(0.13957,xR))};
   ...
   data_O.plotOn(frame, Binning(bin));
   ...
   Int_t npar = model.getParameters(data_O)->selectByAttrib("Constant",kFALSE)->getSize(); //select floating parameters and count their number
   ...
   Ntot_para.Form("N_{sig} = %.2f #pm %.2f", Nsig_O.getValV(), Nsig_O.getError());
   ...
   legend->AddEntry(frame->FindObject("data_O"),"data","pe");
   ...
   w->writeToFile("Ds2D0enue_Analysis/05-ROOT/Fit_Parameter/rf04_OtherPara.root");
   //=========================================================================================================================================================================//
   c->SaveAs("Ds2D0enue_Analysis/09-Images/rf04_DeltaM_Other.png");
