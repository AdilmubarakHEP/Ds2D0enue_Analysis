...
void rf00_Signal()
{
   ...
   // --------------------------------------------------------------
   TFile *file = new TFile("/group/belle/users/amubarak/03-ML/BkgBDT/Ds2D0enu-Signal_withBkgBDT.root");
   ...
   RooDataSet data("data", "dataset with mass", tree, RooArgSet(x,isSignal,BS));
   ...
   // // Method 1:
   // //============================
   // RooRealVar edgeL  ("edgeL","left edge",     0.045, 0.030, 0.060);
   // RooRealVar widthL ("widthL","left smear",   0.006, 0.002, 0.020);
   // RooRealVar edgeR  ("edgeR","right edge",    0.094, 0.080, 0.110);
   // RooRealVar widthR ("widthR","right smear",  0.014, 0.004, 0.040);

   // RooGenericPdf SignalModel(
   // "SignalModel",
   // "0.5*(erf((@0-@1)/@2) - erf((@0-@3)/@4))",
   // RooArgList(x, edgeL, widthL, edgeR, widthR)
   // );
   ...
   // // Method 3:
   // //============================
   // // Non-parametric template from your selected signal
   // RooKeysPdf kpdf("kpdf","keys", x, *filteredData, RooKeysPdf::MirrorBoth, 2.0); // rho≈2 matches your smooth curve well

   // // Light extra smearing
   // RooRealVar mG("mG","G mean",  0.0);
   // RooRealVar sG("sG","G σ",     0.004, 0.000001, 0.012);
   // RooGaussian g("g","G", x, mG, sG);

   // // Convolution
   // x.setBins(4096,"cache");
   // RooFFTConvPdf SignalModel("SignalModel","keys⊗G", x, kpdf, g);
   ...
   RooAddPdf model("model", "sig + bkg", RooArgList(SignalModel, bkg), RooArgList(nsig, nbkg));
   //=========================================================================================================================================================================//
   ...
   model.plotOn(frame, Components(SignalModel),   LineStyle(kDashed), LineColor(kRed+2),    RooFit::Name("Peak"));
   ...
   Ntot_para.Form("N_{sig} = %.2f #pm %.2f", nsig.getValV(), nsig.getError());
   ...
   // X-axis
   TString x_axis ("");
   x_axis.Form("#Delta m_{e}[GeV/c^{2}]");
   ...
   RooWorkspace *w = new RooWorkspace("w_Sig", "workspace");
   ...
   w->writeToFile("/home/belle2/amubarak/Ds2D0enue_Analysis/05-ROOT/00-e_fit/Roofit_Ch1/Fit_Parameter/rf00_Signal.root");
   //=========================================================================================================================================================================//
   c->SaveAs("/home/belle2/amubarak/Ds2D0enue_Analysis/05-ROOT/00-e_fit/Roofit_Ch1/Fit_Image/rf00_Signal.png");

   cout << endl << "Efficiency = " << nsig.getValV()/(2000000) << " +/- " << nsig.getError()/(200000) << endl;
   //=========================================================================================================================================================================//
   // Final Fit Status
   //---------------------------
