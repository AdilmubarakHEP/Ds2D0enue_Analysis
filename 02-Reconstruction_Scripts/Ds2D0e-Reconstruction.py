...
MODE_CFG = {
    "kmpip":    {"d0list": "D0:kmpip",    "dslist": "D_s+:Ch1",
                 "infile": "/group/belle/users/amubarak/00-Generation/Signal/ccbarDs+EventGeneration_Mode1.root"},
    "kmpippi0": {"d0list": "D0:kmpippi0", "dslist": "D_s+:Ch2",
                 "infile": "/group/belle/users/amubarak/00-Generation/Signal/ccbarDs+EventGeneration_Mode2.root"},
    "km3pi":    {"d0list": "D0:km3pi",    "dslist": "D_s+:Ch3",
                 "infile": "/group/belle/users/amubarak/00-Generation/Signal/ccbarDs+EventGeneration_Mode3.root"},
}
...
D0kmpippi0_Cut = "-0.20 <= dM <= 0.15 and useCMSFrame(p) > 2.5"
D0km3pi_Cut = "-0.04 <= dM <= 0.04 and useCMSFrame(p) > 2.5"

# D0_Cut = "[-0.23 <= dM <= -0.03 or 0.03 <= dM <= 0.23] and useCMSFrame(p) > 2.5"
Ds_D0kmpip_Cut = "massDifference(0) <= 0.5 and -0.04 <= daughter(0,dM) <= 0.04"
Ds_D0kmpippi0_Cut = "massDifference(0) <= 0.5 and -0.20 <= daughter(0,dM) <= 0.15"
Ds_D0km3pi_Cut = "massDifference(0) <= 0.5 and -0.04 <= daughter(0,dM) <= 0.04"
...
def argparser():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Running on gbasf2 grid: input files provided via basf2 -i flag, not --infile."
    )
    return parser
...
def Reconstruction_Variable(
    ...
    *, is_data
    ...
    # Additional MC-only variables that appear in cuts/daughters
    mc_only_D0_vars = ["isSignal", "nMCDaughters"]
    mc_only_D0_cuts = []  # Will be filled per mode
    mc_only_daughter_vars = ["isSignal", "mcPDG", "genMotherPDG"]

    if is_data:
        truth_D0 = []  # be strict on data to avoid missing-var crashes
        mc_only_D0_vars = []
        mc_only_D0_cuts = []
        mc_only_daughter_vars = ["isSignal"]  # isSignal works on data, returns 0
    ...
    if mode == "kmpippi0":
        # Build MC-only cuts for this mode
        if not is_data:
            mc_only_D0_cuts = ["passesCut(abs(D0_kmpippi0_nMCDaughters)==2)",
                               "passesCut(abs(D0_kmpippi0_nMCDaughters)==3)",
        ...
        D0_kmpippi0_vars += vu.create_aliases_for_selected(
            ["M","dM","useCMSFrame(E)","useCMSFrame(p)","cosAngleBetweenMomentumAndVertexVector",
             ...
             "flightDistance"] + mc_only_D0_vars + ['ifNANgiveX(isSignal,0)'] +
             mc_only_D0_cuts + ['D0Mode','Dbar0Mode'] + truth_D0,
        ...
        # Build daughter variables conditionally
        pi0_daughter_vars = []
        for var in mc_only_daughter_vars:
            pi0_daughter_vars.extend([f"daughter(0, {var})", f"daughter(1, {var})"])

        pi0_mc_cut = ["passesCut(pi0_kmpippi0_mcPDG==111)"] if not is_data else []

        D0_kmpippi0_vars += vu.create_aliases_for_selected(
            ["M","InvM","E","E1","E2","max(E1, E2)","energyAsymmetry"] +
             pi0_daughter_vars + pi0_mc_cut + mc_only_D0_vars + truth_D0,
            decay_string=f'D0:kmpippi0 -> K- pi+ ^pi0:{pi0_list}',
            prefix=['pi0_kmpippi0'])
...
def create_reconstruction_path(mode, infile, outfile, pi0_list="eff50_May2020", truth=False, data=False, no_electron_id=False, wrong_charge=False):
    ...
    # When running on gbasf2, infile is empty ("") and input is provided via basf2 -i flag
    # For local execution, verify file exists first
    if infile and not os.path.exists(infile):
        raise FileNotFoundError(f"Input not found: {infile}")

    # Pass filename=infile (empty string "" for gbasf2, basf2 uses -i flag automatically)
    ma.inputMdst(filename=infile, path=path)
...
if __name__ == '__main__':
    ...
    # - explicit --infile: use that
    # - explicit --gbasf2: empty string (basf2 handles -i flag)
    # - default from MODE_CFG: use if file exists, otherwise assume gbasf2 mode
    if args.infile:
        infile = args.infile
    elif args.gbasf2:
        infile = ""  # basf2 will use files from -i flag
    else:
        default_infile = cfg["infile"]
        if os.path.exists(default_infile):
            infile = default_infile
        else:
            # Default file doesn't exist -> we're on gbasf2 grid
            infile = ""

    # for data (non-gbasf2), require explicit --infile since there is no default
    if args.data and not args.infile and infile:
    ...
    if not infile:
        # gbasf2/grid: use relative path (file saved in job working dir, transferred back)
        if args.mode == "kmpippi0":
            outfile = f"Ds2D0e_{args.mode}_{args.pi0_list}{cs_suffix}.root"
        else:
            outfile = f"Ds2D0e_{args.mode}{cs_suffix}.root"
    else:
        # Local: use full path
