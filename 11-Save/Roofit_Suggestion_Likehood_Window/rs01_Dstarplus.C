...
void rs01_Dstarplus()
{
   ...
   RooDataSet *data_Dstarplus = (RooDataSet*)data.reduce("Ds_extraInfo_BkgBDT >= 0.531 && Ds_diff_D0pi >= 0.16 && Ds_diff_D0pi <= 0.45");
   ...
   // RooRealVar x00_DstarPBkg("x00_DstarPBkg","x00_DstarPBkg", 1.4443e-01, 0.13, 0.2);
   // RooRealVar a0_DstarPBkg("a0_DstarPBkg","a0_DstarPBkg", 4.6274e-01, 4.0e-01, 6.0e-01);
   // RooRealVar b0_DstarPBkg("b0_DstarPBkg","b0_DstarPBkg", 3.4348e+00, 2.0e+00, 4.0e+00);    
   ...
   // RooRealVar x01_DstarPBkg("x01_DstarPBkg","x01_DstarPBkg", 1.5882e-01, 0.13, 0.2);
   // RooRealVar a1_DstarPBkg("a1_DstarPBkg","a1_DstarPBkg", 2.2105e-01, 2.0e-01, 4.0e-01);
   // RooRealVar b1_DstarPBkg("b1_DstarPBkg","b1_DstarPBkg", 1.2147e+02, 0.0, 500.);     
   ...
   // RooRealVar x02_DstarPBkg("x02_DstarPBkg","x02_DstarPBkg", 1.3076e-01, 0.13, 0.2);
   // RooRealVar a2_DstarPBkg("a2_DstarPBkg","a2_DstarPBkg", 2.2671e+00, 1.0e+00, 3.0e+00);
   // RooRealVar b2_DstarPBkg("b2_DstarPBkg","b2_DstarPBkg", 1.4435e+01, 1.0e+01, 3.0e+01);     
   ...
   // // Add the components
   // RooRealVar f0_DstarPBkg("f0_DstarPBkg", "f0", 9.9990e-01, 0, 1);
   // RooRealVar f1_DstarPBkg("f1_DstarPBkg", "f1", 1.8454e-02, 0, 1);
   // RooRealVar f2_DstarPBkg("f2_DstarPBkg", "f2", 9.9907e-01, 0, 1);
   ...
   RooRealVar c0_DstarPBkg("c0_DstarPBkg", "coefficient #0", 5.3475e-02, -1.0, 1.0);
   RooRealVar c1_DstarPBkg("c1_DstarPBkg", "coefficient #1", -2.9033e-01, -1.0, 1.0);
   RooRealVar c2_DstarPBkg("c2_DstarPBkg", "coefficient #2",  4.4738e-02, -1.0, 1.0);
   RooRealVar c3_DstarPBkg("c3_DstarPBkg", "coefficient #3", 3.1036e-02, -1.0, 1.0);
   RooRealVar c4_DstarPBkg("c4_DstarPBkg", "coefficient #4", -3.5671e-02, -1.0, 1.0);
   RooRealVar c5_DstarPBkg("c5_DstarPBkg", "coefficient #5",  1.3992e-02, -1.0, 1.0);
   RooRealVar c6_DstarPBkg("c6_DstarPBkg", "coefficient #6", -1.3324e-02, -1.0, 1.0);
   RooRealVar c7_DstarPBkg("c7_DstarPBkg", "coefficient #7",  4.8014e-03, -1.0, 1.0);
   RooRealVar c8_DstarPBkg("c8_DstarPBkg", "coefficient #8",  5.1901e-03, -1.0, 1.0);
   c0_DstarPBkg.setConstant(true);
   c1_DstarPBkg.setConstant(true);
   c2_DstarPBkg.setConstant(true);
   c3_DstarPBkg.setConstant(true);
   c4_DstarPBkg.setConstant(true);
   c5_DstarPBkg.setConstant(true);
   c6_DstarPBkg.setConstant(true);
   c7_DstarPBkg.setConstant(true);
   c8_DstarPBkg.setConstant(true);
   RooChebychev CombBkgModel("CombBkgModel", "D0BkgModel", DeltaM, RooArgList(c0_DstarPBkg,c1_DstarPBkg,c2_DstarPBkg,c3_DstarPBkg,c4_DstarPBkg,c5_DstarPBkg,c6_DstarPBkg,c7_DstarPBkg,c8_DstarPBkg));

   // Total Model
   // Add the components
   RooRealVar f_DstarPBkg_Peak("f_DstarPBkg_Peak", "f0", 1.4691e-06, 0, 1);
   RooRealVar f_DstarPBkg_Comb("f_DstarPBkg_Comb", "f1", 5.3635e-04, 0, 1);
   ...
   // --------------------------------------------------------------
   RooRealVar Nsig_DstarPBkg("Nsig_DstarPBkg", "#Signal events", 9.0157e+04, 8.0e+03, 10.0e+04);
   // RooAddPdf model("model", "g+a", RooArgList(PeakingBkgModel), RooArgList(Nsig_PBkg));
   RooAddPdf model("model", "g+a", RooArgList(DstarPModel), RooArgList(Nsig_DstarPBkg));
   //=========================================================================================================================================================================//
   ...
   *data_Dstarplus->plotOn(frame, Binning(50));
   ...
   frame->GetYaxis()->SetRangeUser(0,3000);
   ...
   Nsig_para.Form("N_{D^{*+}} = %.2f #pm %.2f", Nsig_DstarPBkg.getValV(), Nsig_DstarPBkg.getError());

   legend->AddEntry(frame->FindObject("data"),"data","pe");
   legend->AddEntry((TObject *)0, chi2, "");
   legend->AddEntry("model", "D^{*+} #rightarrow D^{0} #pi^{+}", "pl" );
   ...
   w->saveSnapshot("reference_fit_Dstarp_Bkg", *params, true);
   ...
   w->writeToFile("/home/belle2/amubarak/Ds2D0enue_Analysis/10-Save/Roofit_Suggestion_Likehood_Window/Fit_Parameter/rs01_DstarP.root");
   //=========================================================================================================================================================================//
   c->SaveAs("/home/belle2/amubarak/Ds2D0enue_Analysis/10-Save/Roofit_Suggestion_Likehood_Window/Fit_Image/rs01_DeltaM_DstarP.png");
   //=========================================================================================================================================================================//
   // Final Fit Status
   //---------------------------
