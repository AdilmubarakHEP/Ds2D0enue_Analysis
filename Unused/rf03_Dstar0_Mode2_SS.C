...
void rf03_Dstar0_Mode2_SS()
{
   ...
   // I m p o r t s   D a t a   f r o m   t r e e   b r a n c h:
   // --------------------------------------------------------------
   TFile *file = new TFile("/home/belle2/amubarak/C01-Simulated_Events/Dstar0/Dstar0-Background_Mode_2.root");
   ...
   // RooDataSet *data_Mode2 = (RooDataSet*)data.reduce("Ds_extraInfo_BkgBDT >= 0.531 && Ds_diff_D0pi >= 0.1 && Ds_diff_D0pi <= 0.55");
   RooDataSet *data_Mode2 = (RooDataSet*)data.reduce("Ds_extraInfo_BkgBDT >= 0.531 && Ds_ifNANgiveX_isSignal_5!=1 && Ds_diff_D0pi >= 0.1 && Ds_diff_D0pi <= 0.55");
   //=========================================================================================================================================================================//
   // B u i l d   M o d e l
   // -------------------------------------------------------------- 
   // Peaking 
   ...
   // RooRealVar x00_Mode2("x00_Mode2","x00_Mode2", 1.4300e-01, xL, 0.2);
   // RooRealVar a0_Mode2("a0_Mode2","a0_Mode2", 3.5004e+00, 1.0e+00, 5.0e+00);
   // RooRealVar b0_Mode2("b0_Mode2","b0_Mode2", 8.5644e+01, 2.0e+01, 9.0e+01);    
   // x00_Mode2.setConstant(true);
   // a0_Mode2.setConstant(true);
   // b0_Mode2.setConstant(true);
   // RooFormulaVar x00prime_Mode2("#x00prime_Mode2", "x[0] < x[1] ? x[0] : x[1]", RooArgList(DeltaM, x00_Mode2));     
   // RooFormulaVar gamma0_Mode2("#gamma0_Mode2", "a0_Mode2 + 1", RooArgList(a0_Mode2));    
   // RooFormulaVar beta0_for_gamma_Mode2("beta0_for_gamma_Mode2", "1./b0_Mode2", RooArgList(b0_Mode2));    
   // RooGamma GammaModel0_Mode2("GammaModel0_Mode2", "Gamma pdf", DeltaM, gamma0_Mode2, beta0_for_gamma_Mode2, x00prime_Mode2);

   // // Second Gamma Function
   // RooRealVar x01_Mode2("x01_Mode2","x01_Mode2", 1.3959e-01, xL, 0.2);
   // RooRealVar a1_Mode2("a1_Mode2","a1_Mode2", 7.2477e+00, 2.0e+00, 9.0e+00);
   // RooRealVar b1_Mode2("b1_Mode2","b1_Mode2", 9.2038e+01, 5.0e+01, 10.0e+01);     
   // x01_Mode2.setConstant(true);
   // a1_Mode2.setConstant(true);
   // b1_Mode2.setConstant(true);
   // RooFormulaVar x01prime_Mode2("#x01prime_Mode2", "x[0] < x[1] ? x[0] : x[1]", RooArgList(DeltaM, x01_Mode2));     
   // RooFormulaVar gamma1_Mode2("#gamma1_Mode2", "a1_Mode2 + 1", RooArgList(a1_Mode2));    
   // RooFormulaVar beta1_for_gamma_Mode2("beta1_for_gamma_Mode2", "1./b1_Mode2", RooArgList(b1_Mode2));    
   // RooGamma GammaModel1_Mode2("GammaModel1_Mode2", "Gamma pdf", DeltaM, gamma1_Mode2, beta1_for_gamma_Mode2, x01prime_Mode2);

   // // Second Gamma Function
   // RooRealVar x02_Mode2("x02_Mode2","x02_Mode2", 1.0322e-01, xL, 0.2);
   // RooRealVar a2_Mode2("a2_Mode2","a2_Mode2", 5.4106e-01, 2.0e-01, 7.0e-01);
   // RooRealVar b2_Mode2("b2_Mode2","b2_Mode2", 5.5414e+01, 1.0e+01, 8.0e+01);     
   // x02_Mode2.setConstant(true);
   // a2_Mode2.setConstant(true);
   // b2_Mode2.setConstant(true);
   // RooFormulaVar x02prime_Mode2("#x01prime_Mode2", "x[0] < x[1] ? x[0] : x[1]", RooArgList(DeltaM, x02_Mode2));     
   // RooFormulaVar gamma2_Mode2("#gamma2_Mode2", "a2_Mode2 + 1", RooArgList(a2_Mode2));    
   // RooFormulaVar beta2_for_gamma_Mode2("beta2_for_gamma_Mode2", "1./b2_Mode2", RooArgList(b2_Mode2));    
   // RooGamma GammaModel2_Mode2("GammaModel2_Mode2", "Gamma pdf", DeltaM, gamma2_Mode2, beta2_for_gamma_Mode2, x02prime_Mode2);

   // // Add the components
   // RooRealVar f0_Mode2("f0_Mode2", "f0", 2.1712e-01, 0.0e-01, 5.0e-01);
   // RooRealVar f1_Mode2("f1_Mode2", "f1", 1.4434e-01, 0.0e-01, 5.0e-01);
   // RooRealVar f2_Mode2("f2_Mode2", "f1", 2.3999e-01, 0.0e-01, 5.0e-01);
   // f0_Mode2.setConstant(true);
   // f1_Mode2.setConstant(true);
   // f2_Mode2.setConstant(true);
	// RooAddPdf Dstar0Mode2Model("Dstar0Mode1Model","g1+g2",RooArgList(GammaModel0_Mode2,GammaModel1_Mode2,GammaModel2_Mode2), RooArgList(f0_Mode2,f1_Mode2,f2_Mode2));

   // -------------------------------------------------------------- 
   // Combinatorial Part
   // -------------------------------------------------------------- 
   // First Attempt:
   RooRealVar B0_Mode2("B0_Mode2", "B0_Mode2", -40., 20.);
   RooRealVar P0_Mode2("P0_Mode2", "P0_Mode2", 0., 5.);
   RooGenericPdf comb_Mode2("comb_Mode2","Generic PDF","(Ds_diff_D0pi - 0.13953 > 0 ? ( ((Ds_diff_D0pi-0.13953)^P0_Mode2)*exp( B0_Mode2*(Ds_diff_D0pi-0.13953))) : 0.0)", RooArgSet( DeltaM, B0_Mode2, P0_Mode2));
   // Second Attempt:
   // RooRealVar c0("c0", "coefficient #0", -1, 1);
   // RooRealVar c1("c1", "coefficient #1", -1, 1);
   // RooRealVar c2("c2", "coefficient #2", -1, 1);
   // RooPolynomial comb_Mode2("comb_Mode2", "background p.d.f.", DeltaM, RooArgList(c0,c1,c2));
   // Third Attempt:
   // // First Gamma Function
   ...
   // RooAddPdf comb_Mode2("comb_Mode2","g0+g1+g2",RooArgList(GammaModel0_comb,GammaModel1_comb,GammaModel2_comb), RooArgList(f0_comb,f1_comb,f2_comb));

   // C o n s t r u c t   a   S i g na l   a n d   B a c k g r o u n d   P D F:
   // --------------------------------------------------------------
   // RooRealVar Nsig_Mode2("nsig_Mode2", "#Signal events", 0, 8.0e+05);
   RooRealVar Nbkg_Mode2("nbkg_Mode2", "#background events", 0, 5.0e+05);
   // RooAddPdf model("model", "g+a", RooArgList(PeakingBkgModel), RooArgList(Nsig_Mode2));
   RooAddPdf model("model", "g+a", RooArgList(comb_Mode2), RooArgList(Nbkg_Mode2));
   //=========================================================================================================================================================================//
   ...
   // g2frac_Mode2.setVal();
   // g3frac_Mode2.setVal();
   // sigmean1_Mode2.setVal();
   // sigmean2_Mode2.setVal();
   // sigmean3_Mode2.setVal();
   // sigwidth1_Mode2.setVal();
   // sigwidth2_Mode2.setVal();
   // sigwidth3_Mode2.setVal();

   // The code below removes the range and fixes the value.
   // B0_Mode2.setConstant(true);
   // P0_Mode2.setConstant(true);
   // g1frac_Mode2.setConstant(true);
   // g2frac_Mode2.setConstant(true);
   // g3frac_Mode2.setConstant(true);
   // sigmean1_Mode2.setConstant(true);
   // sigmean2_Mode2.setConstant(true);
   // sigmean3_Mode2.setConstant(true);
   // sigwidth1_Mode2.setConstant(true);
   // sigwidth2_Mode2.setConstant(true);
   // sigwidth3_Mode2.setConstant(true);
   ...
   std::unique_ptr<RooFitResult> r{model.fitTo(*data_Mode2, Save(), PrintLevel(-1))};
   ...
   *data_Mode2->plotOn(frame, Binning(bin));
   model.plotOn(frame, Components(comb_Mode2),       LineStyle(kDashed), LineColor(kRed+1), RooFit::Name("Background"));  // Background in deep red
   // model.plotOn(frame, Components(Dstar0Mode2Model), LineStyle(kDashed), LineColor(kBlue+1), RooFit::Name("Signal"));  // Signal in deep blue
   ...
   Int_t npar = model.getParameters(*data_Mode2)->selectByAttrib("Constant",kFALSE)->getSize(); //select floating parameters and count their number
   ...
   // Float_t N_value = Nsig_Mode2.getValV();
   ...
   // Nsig_para.Form("N_{sig} = %.2f #pm %.2f", Nsig_Mode2.getValV(), Nsig_Mode2.getError());
   Nbkg_para.Form("N_{bkg} = %.2f #pm %.2f", Nbkg_Mode2.getValV(), Nbkg_Mode2.getError());
   ...
   // First Try:
   // B0_Mode2.setConstant(true);
   // B0_Mode2.removeError();
   // P0_Mode2.setConstant(true);
   // P0_Mode2.removeError();
   // g1frac_Mode2.setConstant(true);
   // g1frac_Mode2.removeError();
   // g2frac_Mode2.setConstant(true);
   // g2frac_Mode2.removeError();
   // g3frac_Mode2.setConstant(true);
   // g3frac_Mode2.removeError();
   // sigmean1_Mode2.setConstant(true);
   // sigmean1_Mode2.removeError();
   // sigmean2_Mode2.setConstant(true);
   // sigmean2_Mode2.removeError();
   // sigmean3_Mode2.setConstant(true);
   // sigmean3_Mode2.removeError();
   // sigwidth1_Mode2.setConstant(true);
   // sigwidth1_Mode2.removeError();
   // sigwidth2_Mode2.setConstant(true);
   // sigwidth2_Mode2.removeError();
   // sigwidth3_Mode2.setConstant(true);
   // sigwidth3_Mode2.removeError();
   // Second Try:
   ...
   // RooWorkspace *w = new RooWorkspace("w_Mode2", "workspace");
   // // Import model and all its components into the workspace
   // w->import(Dstar0Mode2Model);
   ...
   // RooArgSet *params = Dstar0Mode2Model.getVariables();
   ...
   // w->saveSnapshot("reference_fit_Mode2", *params, true);
   ...
   // w->writeToFile("/home/belle2/amubarak/Ds2D0enue_Analysis/05-ROOT/Roofit_New/Fit_Parameter/rf02-Dstar0_Mode2.root");
   //=========================================================================================================================================================================//
   c->SaveAs("/home/belle2/amubarak/Ds2D0enue_Analysis/05-ROOT/Roofit_New/Fit_Image/rf02-Dstar0_Mode2.png");
