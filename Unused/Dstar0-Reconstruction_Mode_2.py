...
ma.reconstructDecay(decayString='pi0:dalitz -> e+:corrected ... ?gamma',
                    ...
                    path=my_path)
ma.applyCuts('pi0:dalitz', '', path=my_path)

# Reconstruct D*0
#-----------------------------------------
ma.reconstructDecay(decayString='D*0 -> [D0:kpi -> K-:loose pi+:loose ] [pi0:dalitz -> e+:corrected ... ?gamma]', 
                    cut=Dstar0_Cut,
                    chargeConjugation=ChargeC,
                    allowChargeViolation=True,
                    path=my_path)
...
ma.fillSignalSideParticleList(outputListName='e+:sig',
                              decayString='D*0 -> [D0:kpi -> K-:loose pi+:loose ] [pi0:dalitz -> ^e+:corrected ... ?gamma]',
                              path=roe_path)
...
Dstar0_vars += vu.create_aliases_for_selected(
                list_of_variables=['electronID','binaryPID(11,211)',
                                   ...
                                   + tracks,
                decay_string='D*0 -> [D0:kpi -> K-:loose pi+:loose ] [pi0:dalitz -> ^e+:corrected ... ?gamma]',
                prefix=['e'])
...
# # Overall Cut:
# #------------------
# ma.applyCuts('D*0:Ch1', "Dstar0_1_isSignal==1", path=my_path)
