...
void rs01_Dstarplus_Zoom()
{
   ...
   RooCrystalBall CrystallB("CrystallB", "sig4", DeltaM, sig_mean,sig_sigmaL,sig_sigmaR,sig_alphaL,sig_nL,sig_alphaR,sig_nR);
	...
	RooRealVar sigmean0_PBkg("sigmean0_PBkg", "mean", 1.3586e-01, 0.135, 0.155);
	RooRealVar sigwidth0_PBkg("sigwidth0_PBkg", "width", 2.8301e-02, 0, 0.2);
    sigmean0_PBkg.setConstant();
    sigwidth0_PBkg.setConstant();
	RooGaussModel gaussm0_PBkg("gaussm0_PBkg", "Signal PDF", DeltaM, sigmean0_PBkg, sigwidth0_PBkg);

	RooRealVar sigmean1_PBkg("sigmean1_PBkg", "mean", 1.6796e-01, 0.135, 0.17);
	RooRealVar sigwidth1_PBkg("sigwidth1_PBkg", "width", 5.0222e-06, 0, 0.1);
    sigmean1_PBkg.setConstant();
    sigwidth1_PBkg.setConstant();
	RooGaussModel gaussm1_PBkg("gaussm1_PBkg", "Signal PDF", DeltaM, sigmean1_PBkg, sigwidth1_PBkg);

	RooRealVar sigmean2_PBkg("sigmean2_PBkg", "mean", 1.4986e-01, 0.135, 0.155);
	RooRealVar sigwidth2_PBkg("sigwidth2_PBkg", "width", 4.1084e-03, 0, 0.1);
    sigmean2_PBkg.setConstant();
    sigwidth2_PBkg.setConstant();
	RooGaussModel gaussm2_PBkg("gaussm2_PBkg", "Signal PDF", DeltaM, sigmean2_PBkg, sigwidth2_PBkg);

   // // Define Novosibirsk parameters
   // RooRealVar peak("peak", "Peak position", 0.145, 0.140, 0.149); // Near the D*+ - D0 mass difference
   // RooRealVar width("width", "Width", 0.0, 40.0);
   // RooRealVar tail("tail", "Tail parameter", -40.0, 0.0); // Controls the asymmetry
   // // // Novosibirsk function formula
   // // RooGenericPdf novosibirsk("novosibirsk", "exp(-0.5 * (log(1 + (tail*(Ds_diff_D0pi - peak)/width))^2) / tail^2)",
   // //                            RooArgList(DeltaM, peak, width, tail));
   // RooNovosibirsk novosibirsk("novosibirsk","novosibirsk",DeltaM,peak,width,tail);

   RooRealVar f0("f0", "Fraction #1", 9.2342e-01, 0, 1.0);
   RooRealVar f1("f1", "Fraction #1", 1.0490e-07, 0, 1.0);
   RooRealVar f2("f2", "Fraction #1", 1.2650e-08, 0, 1.0);
   RooRealVar f3("f3", "Fraction #1", 1.8303e-02, 0, 1.0);
   f0.setConstant();
   f1.setConstant();
   f2.setConstant();
   f3.setConstant();
   // RooAddPdf PeakingBkgModel("PeakingBkgModel", "Combined Reverse Argus", RooArgList(CrystallB,novosibirsk), RooArgList(f0,f1));
   RooAddPdf PeakingBkgModel("PeakingBkgModel", "Combined Reverse Argus", RooArgList(CrystallB,gaussm0_PBkg,gaussm1_PBkg,gaussm2_PBkg), RooArgList(f0,f1,f2,f3));
   ...
   // RooPolynomial CombBkgModel("CombBkgModel", "background p.d.f.", DeltaM, RooArgList(c0,c1,c2,c3));
   ...
   // RooRealVar c0("c0", "Cutoff", xR, 15);
   // RooRealVar m0("m0", "m0", -100.0, 0.0);
   // RooRealVar p0("p0", "p0", 0.0, 10.0);
   // // RooFormulaVar x_flip0("x_flip0", "c0 - Ds_diff_D0pi", RooArgList(c0, DeltaM));
   // RooArgusBG Argus_0("Argus_0", "1st Argus Function", DeltaM, c0, m0, p0);

   // // Define Reverse Argus function parameters
   // RooRealVar c1("c1", "Cutoff", xR, 15);
   // RooRealVar m1("m1", "m0", -100.0, 0.0);
   // RooRealVar p1("p1", "p0", 0.0, 10.0);
   // // RooFormulaVar x_flip0("x_flip0", "c0 - Ds_diff_D0pi", RooArgList(c0, DeltaM));
   // RooArgusBG Argus_1("Argus_1", "1st Argus Function", DeltaM, c1, m1, p1);

   // // Combine Reverse Argus Functions
   // RooRealVar f0("f0", "Fraction #1", 0, 1.0);
   // RooAddPdf CombBkgModel("CombBkgModel", "Combined Reverse Argus", RooArgList(Argus_0,Argus_1), RooArgList(f0));

   // // // Combine Reverse Argus Functions
   // // RooRealVar f0("f0", "Fraction #1", 1.0);
   // // First Gamma Function
   // RooRealVar x00_DstarPBkg("x00_DstarPBkg","x00_DstarPBkg", 0.13, 0.15);
   // RooRealVar a0_DstarPBkg("a0_DstarPBkg","a0_DstarPBkg", 0.0, 10.);
   // RooRealVar b0_DstarPBkg("b0_DstarPBkg","b0_DstarPBkg", 0.0, 20.);    
   ...
   // RooGamma CombBkgModel("CombBkgModel", "Gamma pdf", DeltaM, gamma0_DstarPBkg, beta0_for_gamma_DstarPBkg, x00prime_DstarPBkg);

   // // Total Model
   // // Add the components
   // RooRealVar f_DstarPBkg("f_DstarPBkg", "f", 0, 1);
   // // RooRealVar f_DstarPBkg_Comb("f_DstarPBkg_Comb", "f1", 1.4165e-01, 0, 1);
   // // f_DstarPBkg_Peak.setConstant();
   // // f_DstarPBkg_Comb.setConstant();
   // RooAddPdf DstarPModel("DstarPModel","g1+g2",RooArgList(PeakingBkgModel,CombBkgModel), RooArgList(f_DstarPBkg));

   // First Gamma Function
   RooRealVar x00_DstarPBkg("x00_DstarPBkg","x00_DstarPBkg", 1.3980e-01, 0.135, 0.2);
   RooRealVar a0_DstarPBkg("a0_DstarPBkg","a0_DstarPBkg", 3.7829e-01, 2.0e-01, 6.0e-01);
   RooRealVar b0_DstarPBkg("b0_DstarPBkg","b0_DstarPBkg", 1.7939e+00, 1.0e+00, 4.0e+00);    
   // x00_DstarPBkg.setConstant();
   // a0_DstarPBkg.setConstant();
   // b0_DstarPBkg.setConstant();
   RooFormulaVar x00prime_DstarPBkg("#x00prime_DstarPBkg", "x[0] < x[1] ? x[0] : x[1]", RooArgList(DeltaM, x00_DstarPBkg));     
   ...
   RooRealVar x01_DstarPBkg("x01_DstarPBkg","x01_DstarPBkg", 2.3937e-01, 0.135, 0.25);
   RooRealVar a1_DstarPBkg("a1_DstarPBkg","a1_DstarPBkg", 1.0241e+01, 0.0e+01, 3.0e+01);
   RooRealVar b1_DstarPBkg("b1_DstarPBkg","b1_DstarPBkg", 6.0650e+00, 4.0e+00, 8.0e+00);     
   ...
   RooRealVar x02_DstarPBkg("x02_DstarPBkg","x02_DstarPBkg", 1.4278e-01, 0.135, 0.2);
   RooRealVar a2_DstarPBkg("a2_DstarPBkg","a2_DstarPBkg", 1.5341e+00, 1.0e+00, 3.0e+00);
   RooRealVar b2_DstarPBkg("b2_DstarPBkg","b2_DstarPBkg", 1.0385e+01, 0.0e+01, 3.0e+01);     
   ...
   // Add the components
   RooRealVar f0_DstarPBkg("f0_DstarPBkg", "f0", 7.1325e-01, 5.0e-01, 9.0e-01);
   RooRealVar f1_DstarPBkg("f1_DstarPBkg", "f1", 2.0819e-04, 0.0e-04, 4.0e-04);
   RooRealVar f2_DstarPBkg("f2_DstarPBkg", "f2", 9.6634e-01, 7.0e-01, 11.0e-01);
   ...
   RooRealVar Nbkg_PBkg("nbkg_PBkg", "#Signal events", 0.0, 4.0e+05);
   // RooAddPdf model("model", "g+a", RooArgList(PeakingBkgModel), RooArgList(Nsig_PBkg));
   RooAddPdf model("model", "g+a", RooArgList(PeakingBkgModel,CombBkgModel), RooArgList(Nsig_PBkg,Nbkg_PBkg));
   //=========================================================================================================================================================================//
   ...
   *data_Dstarplus->plotOn(frame, Binning(50));
   ...
   model.plotOn(frame, Components(PeakingBkgModel),   LineStyle(kDashed), LineColor(kRed+2),    RooFit::Name("Peak"));
   model.plotOn(frame, Components(CombBkgModel),  LineStyle(kDashed), LineColor(kGreen+3),   RooFit::Name("Comb"));
   ...
   // // Convert unbinned data into binned data
   // RooDataHist* binnedData = data_Dstarplus->binnedClone();

   // RooChi2Var Chi2("Chi2", "Chi-squared", model, *binnedData);
   // double chi2_val = Chi2.getVal();
   // int ndf = binnedData->numEntries() - model.getParameters(*binnedData)->getSize();

   // Double_t chi2ndf = chi2_val / ndf;

   // cout << "NDF = " << ndf << endl;
   // cout << "chi^2/NDF = " << chi2ndf << endl;
   //=========================================================================================================================================================================//
   // P l o t    L a b e l s
   // ---------------------------------------------------------------------------
   // // Add text to frame
   ...
   frame1->GetYaxis()->SetRangeUser(-6,6);
   ...
   RooArgSet *params = PeakingBkgModel.getVariables();
   ...
   // w->writeToFile("/home/belle2/amubarak/Ds2D0enue_Analysis/10-Suggestion/Roofit_Suggestion_Likehood/Fit_Parameter/rs01_DstarP.root");
   //=========================================================================================================================================================================//
   c->SaveAs("/home/belle2/amubarak/Ds2D0enue_Analysis/10-Suggestion/Roofit_Suggestion_Likehood/Fit_Image/rs01_DeltaM_DstarP_Zoom.png");
   //=========================================================================================================================================================================//
   // Final Fit Status
   //---------------------------
