...
b2.conditions.append_globaltag(ma.getAnalysisGlobaltag()) # wait 180ms, doesnt work when confl down, 
...
D0_Cut = "-0.02 <= dM <= 0.02 and useCMSFrame(p) > 2.5"
Ds_Cut = "useAlternativeDaughterHypothesis(massDifference(0), 1:pi+) <= 0.7 and -0.02 <= daughter(0,dM) <= 0.02"
...
# ma.matchMCTruth(list_name='gamma:V0_array', path=my_path) 
...
ma.fillParticleList("e+:uncorrected", cut='', path=my_path)
# ma.tagCurlTracks("e+:uncorrected", selectorType='mva', ptCut=0.6, mcTruth=True, expert_filename='test.root', path=my_path)
# vm.addAlias("isCurl", "extraInfo(isCurl)") 
# ma.replaceMass("e+:uncorrected", particleLists="e+:uncorrected", pdgCode=211, path=my_path) # Does not recalculate anything, still e mass
#/================================================================================================================================/
# # Kaon Count
# #---------------
# # Kshorts
# stdV0s.stdKshorts(path=my_path)
# stdV0s.goodBelleKshort(path=my_path)
# # ma.applyCuts('K_S0:merged', "daughter(0,pionID)>=0.1 and daughter(1,pionID)>=0.1 and 0.49<=M<=0.505", path=my_path)

# ma.fillParticleList('K+:plus', cut="kaonID > 0.5 and charge==1", path=my_path)
# ma.fillParticleList('K-:minus', cut="kaonID > 0.5 and charge==-1", path=my_path)
#/================================================================================================================================/
...
# ma.tagCurlTracks("e+:corrected", selectorType='mva', ptCut=0.6, mcTruth=True, expert_filename='test.root', path=my_path)
#/================================================================================================================================/
# MVA
#------------------------
# Photon
...
vx.treeFit('D_s+', 
           ...
           updateAllDaughters=False, 
           path=my_path)
ma.applyCuts("D_s+", Ds_Cut, path=my_path)
# Save some Variables Before IP Constraint
ma.variablesToExtraInfo('D_s+', variables={"chiProb": "chiProb_noIP", "flightDistance": "flightDistance_noIP"}, path=my_path)
...
#            conf_level=-1,
#            path=my_path)
ma.applyCuts("D_s*+", Ds_star_Cut, path=my_path)
...
ma.matchMCTruth(list_name='D_s+', path=my_path) 
ma.matchMCTruth(list_name='D_s*+', path=my_path) 
#/================================================================================================================================/
...
ma.appendROEMasks("D_s+", [roe_mask], path=my_path)  # [E40]
...
# # execute roe_path for each RestOfEvent in the event
# my_path.for_each('RestOfEvent', 'RestOfEvents', roe_path_Ds_star)
...
#   sourceParticleListName='D_s+', useMissing = True, path=my_path)
#/================================================================================================================================/
# Other Tools:
#-------------------
# ma.calculateDistance('e+:corrected', 'D_s+ -> [^D0:kpi -> K- pi+ ] e+:corrected ?nu', "trackvertex", path=my_path)
# vm.addAlias("CalculatedDistance", "extraInfo(CalculatedDistance)")  # [E30]

# ma.estimateAndAttachTrackFitResult('e+:corrected', path=my_path)
...
Ds_vars += vu.create_aliases_for_selected(
    list_of_variables=["Mode1Veto","Mode2Veto","DstarplusVeto",
                    ...
                    #    'originalDaughter(1, cos(theta))',
                       ...
                       "M_uncorrected",
                       'M_pi',"mcM","mcM_D0e_emass",'mcM_D0e_pimass',"MminusMtrue_D0e_emass","MminusMtrue_D0e_pimass",
                      ...
                      #  + ["nROE_Kplus","nROE_Kminus","nROE_KS0_1","nROE_KS0_2",
                       ...
                       + gamma_ROE + gammaCS_ROE + Dstar0Mode1_ROE + Dstar0Mode2_ROE + Dstarplus_ROE + vpho_ROE
...
# Overall Cut:
#------------------
# ma.applyCuts('K_S0:merged', "daughter(0,pionID) and daughter(1,pionID)", path=my_path)
# ma.applyCuts('D_s+', "Ds_chiProb_rank==1 and Ds_D0_sideband==1", path=my_path)
# ma.applyCuts('D_s+', "e_mcSecPhysProc>=10 and e_mcSecPhysProc<=20 and gammaveto_M>=0.1", path=my_path)
# ma.applyCuts('D_s*+', "gamma_beamBackgroundSuppressionScore>=0.5 and gamma_fakePhotonSuppressionScore>=0.5", path=my_path)
#/==========================================================================================================================/
# Save
#--------
...
# ma.variablesToNtuple('vpho', ['M','daughterAngle(0,1)','useCMSFrame(daughterAngle(0,1))',
#                               'cos(daughterAngle(0,1))','useCMSFrame(cos(daughterAngle(0,1)))'],# + mc_gen_topo(n=200),
#                      filename=output_file, treename='vphotree', path=roe_path)
# ma.variablesToNtuple('K+:ROE', ['M','passesCut(abs(mcPDG)==321)','mcPDG','kaonID'],# + mc_gen_topo(n=200),
#                      filename=output_file, treename='Ktree', path=roe_path)

# ma.variablesToNtuple('K_S0:merged', ['M','passesCut(abs(mcPDG)==310)',
#                                      'mcPDG','daughter(0,pionID)','daughter(1,pionID)'],# + mc_gen_topo(n=200),
#                      filename=output_file, treename='Kshorttree', path=my_path)
# ma.variablesToNtuple('K_S0:legacyGoodKS', ['M','passesCut(abs(mcPDG)==310)',
#                                      'mcPDG','daughter(0,pionID)','daughter(1,pionID)'],# + mc_gen_topo(n=200),
#                      filename=output_file, treename='Kshort2tree', path=my_path) 
# ma.variablesToNtuple('K+:plus', ['M','passesCut(abs(mcPDG)==321)'],# + mc_gen_topo(n=200),
#                      filename=output_file, treename='Ktree', path=my_path) 

# # D_s*+ -> [D_s+ -> [D0:kpi -> K- pi+ ] e+:corrected ?nu] gamma:recon
# ma.variablesToNtuple('D_s*+', Ds_star_vars,
#                      filename=output_file, treename='Ds*tree', path=my_path) 
# D_s+ -> D0 e+ nu_e decay
ma.variablesToNtuple('D_s+', Ds_vars + Event,# + mc_gen_topo(n=200),
