...
void rs05_Combined()
{
   ...
   TFile *f_S = new TFile("/home/belle2/amubarak/Ds2D0enue_Analysis/10-Suggestion/Roofit_Suggestion_Chi/Fit_Parameter/rs00_SignalPara.root");
   TFile *f_DstarP = new TFile("/home/belle2/amubarak/Ds2D0enue_Analysis/10-Suggestion/Roofit_Suggestion_Chi/Fit_Parameter/rs01_DstarP.root");
   TFile *f_Dstar0 = new TFile("/home/belle2/amubarak/Ds2D0enue_Analysis/10-Suggestion/Roofit_Suggestion_Chi/Fit_Parameter/rs02_Dstar0.root");
   TFile *f_D0 = new TFile("/home/belle2/amubarak/Ds2D0enue_Analysis/10-Suggestion/Roofit_Suggestion_Chi/Fit_Parameter/rs03_D0.root");
   TFile *f_O = new TFile("/home/belle2/amubarak/Ds2D0enue_Analysis/10-Suggestion/Roofit_Suggestion_Chi/Fit_Parameter/rs04_OtherPara.root");
   ...
   // Peaking Background
   RooWorkspace *w_DstarP = (RooWorkspace *)f_DstarP->Get("w_Dstarplus");
   ...
   TH1F* h1 = (TH1F*)file->Get("hist_Dstarplus");
   TH1F* h2 = (TH1F*)file->Get("hist_Dstar0");
   TH1F* h3 = (TH1F*)file->Get("hist_D0");
   TH1F* h4 = (TH1F*)file->Get("hist_FakeD0");
   //=========================================================================================================================================================================//
   // D a t a:
   // --------------------------------------------------------------
   // Create a new histogram for the combined data
   TH1F* h_combined = (TH1F*)h1->Clone("h_combined"); // Clone h1 as the base
   h_combined->Add(h2);  // Add hist2
   h_combined->Add(h3);  // Add hist3
   h_combined->Add(h4);  // Add hist4

   // Define the mass variable
   RooRealVar DeltaM("Ds_diff_D0pi", "Mass Difference", xL, xR);

   // Convert combined histogram into RooDataHist for binned fit
   RooDataHist data_combined("data_combined", "Combined dataset", RooArgSet(DeltaM), h_combined);
   //=========================================================================================================================================================================//
   ...
   // //-----------------------
   // // First Gamma Function
   // RooRealVar x00_Dstar0Bkg("x00_Dstar0Bkg","x00_Dstar0Bkg", 1.3949e-01, 0.13, 0.15);
   // RooRealVar a0_Dstar0Bkg("a0_Dstar0Bkg","a0_Dstar0Bkg", 8.6314e-01, 0.0, 12.);
   // RooRealVar b0_Dstar0Bkg("b0_Dstar0Bkg","b0_Dstar0Bkg", 9.5630e+01, 0.0, 100.);    
   ...
   // RooGamma GammaModel0_Dstar0Bkg("GammaModel1_Dstar0Bkg", "Gamma pdf", DeltaM, gamma0_Dstar0Bkg, beta0_for_gamma_Dstar0Bkg, x00prime_Dstar0Bkg);

   // // Second Gamma Function
   // RooRealVar x01_Dstar0Bkg("x01_Dstar0Bkg","x01_Dstar0Bkg", 1.5000e-01, 0.13, 0.15);
   // RooRealVar a1_Dstar0Bkg("a1_Dstar0Bkg","a1_Dstar0Bkg", 7.7622e-01, 0.0, 10.);
   // RooRealVar b1_Dstar0Bkg("b1_Dstar0Bkg","b1_Dstar0Bkg", 6.1159e+00, 0.0, 50.);     
   ...
   // RooGamma GammaModel1_Dstar0Bkg("GammaModel2_Dstar0Bkg", "Gamma pdf", DeltaM, gamma1_Dstar0Bkg, beta1_for_gamma_Dstar0Bkg, x01prime_Dstar0Bkg);

   // // Third Gamma Function
   // RooRealVar x02_Dstar0Bkg("x02_Dstar0Bkg","x02_Dstar0Bkg", 1.5000e-01, 0.13, 0.15);
   // RooRealVar a2_Dstar0Bkg("a2_Dstar0Bkg","a2_Dstar0Bkg", 2.1595e+00, 0.0, 10.);
   // RooRealVar b2_Dstar0Bkg("b2_Dstar0Bkg","b2_Dstar0Bkg", 4.7393e+01, 0.0, 50.);     
   ...
   // RooGamma GammaModel2_Dstar0Bkg("GammaModel3_Dstar0Bkg", "Gamma pdf", DeltaM, gamma2_Dstar0Bkg, beta2_for_gamma_Dstar0Bkg, x02prime_Dstar0Bkg);

   // // Add the components
   // RooRealVar f0_Dstar0Bkg("f0_Dstar0Bkg", "f0", 4.2140e-02, 0, 1);
   // RooRealVar f1_Dstar0Bkg("f1_Dstar0Bkg", "f1", 1.0000e+00, 0, 1);
   // RooRealVar f2_Dstar0Bkg("f2_Dstar0Bkg", "f2", 8.1253e-02, 0, 1);
   // // RooRealVar f3_Dstar0Bkg("f3_Dstar0Bkg", "f3", 0, 1);
   ...
   // RooRealVar f1("f1", "f1", 2.0573e+05, 0.5e+05, 4.0e+05);
   // RooRealVar f2("f2", "f2", 1.7823e+05, 0.5e+05, 4.0e+05);
   // RooRealVar f3("f3", "f3", 7.1232e+04, 5.0e+04, 9.0e+04);
   // RooRealVar f4("f4", "f4", 5.1778e+03, 3.0e+03, 7.0e+03);
   RooRealVar f0("f0", "f0", 0, -50000., 50000000.);
   RooRealVar f1("f1", "f1", 2.0573e+05, 0.5e+05, 4.0e+05);
   RooRealVar f2("f2", "f2", 1.7823e+05, 0.5e+05, 4.0e+05);
   RooRealVar f3("f3", "f3", 7.1232e+04, 5.0e+04, 9.0e+04);
   RooRealVar f4("f4", "f4", 5.1778e+03, -1.0e+10, 7.0e+03);
   // f0.setConstant(true);
   f1.setConstant(true);
   f2.setConstant(true);
   f3.setConstant(true);
   ...
   // RooAddPdf model("model", "s+pbkg+cbkg+fs", RooArgList(*SignalModel,Dstar0BkgModel), RooArgList(f0));
   ...
   RooChi2Var chi2("chi2", "chi2", model, data_combined, DataError(RooAbsData::SumW2));
   //=========================================================================================================================================================================//
   // DeltaM.setVal(0.325);
   ...
   std::unique_ptr<RooFitResult> r{model.chi2FitTo(data_combined, Save(), PrintLevel(-1))};
   //=========================================================================================================================================================================//
   // P l o t 
   //---------
   ...
   data_combined.plotOn(frame, Binning(bin));

   model.plotOn(frame, Components(*SignalModel),     LineStyle(kDashed), LineColor(kBlue+1),     RooFit::Name("Signal"));
   model.plotOn(frame, Components(*DstarPModel),     LineStyle(kDashed), LineColor(kGreen+2),    RooFit::Name("DstarP"));
   model.plotOn(frame, Components(*Dstar0BkgModel),  LineStyle(kDashed), LineColor(kOrange+7),   RooFit::Name("Dstar0"));
   model.plotOn(frame, Components(*D0BkgModel),      LineStyle(kDashed), LineColor(kViolet+3),   RooFit::Name("D0"));
   model.plotOn(frame, Components(*OtherModel),      LineStyle(kDashed), LineColor(kRed+2),      RooFit::Name("Other"));
   ...
   Int_t npar = model.getParameters(data_combined)->selectByAttrib("Constant",kFALSE)->getSize(); //select floating parameters and count their number
   ...
   frame->GetYaxis()->SetRangeUser(0, 15000); // Log scale cannot have 0
   frame->Draw();
   ...
   //-------------------------------
   c->SaveAs("/home/belle2/amubarak/Ds2D0enue_Analysis/10-Suggestion/Roofit_Suggestion_Chi/Fit_Image/rs05_DeltaM_Combined.png");
   //=========================================================================================================================================================================//
   ...
   int ndof = model.getParameters(data_combined)->selectByAttrib("Constant", kFALSE)->getSize(); 
