...
void rs01_Dstarplus()
{
   ...
   TH1F* hh = (TH1F*)file->Get("hist_Dstarplus");
   //=========================================================================================================================================================================//
   // D a t a:
   // --------------------------------------------------------------
   RooRealVar DeltaM("Ds_diff_D0pi", "Mass Difference", xL, xR);
   RooDataHist data_Dstarplus("data_Dstarplus", "dataset with mass", DeltaM, hh);
   //=========================================================================================================================================================================//
   // B u i l d   M o d e l
   // -------------------------------------------------------------- 
   // // First Attempt
   ...
   // Third Gamma Function
   RooRealVar x03_DstarPBkg("x03_DstarPBkg","x03_DstarPBkg", 1.4157e-01, 0.13, 0.15);
   RooRealVar a3_DstarPBkg("a3_DstarPBkg","a3_DstarPBkg", 8.0284e-01, 0.0, 6.);
   RooRealVar b3_DstarPBkg("b3_DstarPBkg","b3_DstarPBkg", 6.0455e+00, 0.0, 20.);     
   x03_DstarPBkg.setConstant();
   a3_DstarPBkg.setConstant();
   b3_DstarPBkg.setConstant();
   RooFormulaVar x03prime_DstarPBkg("#x03prime_S", "x[0] < x[1] ? x[0] : x[1]", RooArgList(DeltaM, x03_DstarPBkg));     
   RooFormulaVar gamma3_DstarPBkg("#gamma3_DstarPBkg", "a3_DstarPBkg + 1", RooArgList(a3_DstarPBkg));    
   RooFormulaVar beta3_for_gamma_DstarPBkg("beta3_for_gamma_DstarPBkg", "1./b3_DstarPBkg", RooArgList(b3_DstarPBkg));    
   RooGamma GammaModel3_DstarPBkg("GammaModel3_DstarPBkg", "Gamma pdf", DeltaM, gamma3_DstarPBkg, beta3_for_gamma_DstarPBkg, x03prime_DstarPBkg);
   ...
   RooAddPdf CombBkgModel("CombBkgModel","g1+g2",RooArgList(GammaModel0_DstarPBkg,GammaModel1_DstarPBkg,GammaModel2_DstarPBkg,GammaModel3_DstarPBkg), RooArgList(f0_DstarPBkg,f1_DstarPBkg,f2_DstarPBkg,f3_DstarPBkg));

   // Total Model
   // Add the components
   RooRealVar f_DstarPBkg_Peak("f_DstarPBkg_Peak", "f0", 1.5763e-01, 0, 1);
   ...
   RooChi2Var chi2("chi2", "chi2", model, data_Dstarplus, DataError(RooAbsData::SumW2));
   //=========================================================================================================================================================================//
   // F i t
   //-------------
   // Perform extended ML fit of data:
   // std::unique_ptr<RooFitResult> r{model.fitTo(data_Dstarplus, Save(), PrintLevel(-1))};
   std::unique_ptr<RooFitResult> r{model.chi2FitTo(data_Dstarplus, Save(), PrintLevel(-1))};
   //=========================================================================================================================================================================//
   ...
   model.plotOn(frame, LineColor(kGreen+2), RooFit::Name("model"));
   //=========================================================================================================================================================================//
   ...
   RooWorkspace *w = new RooWorkspace("w_Dstarplus", "workspace");
   ...
   w->saveSnapshot("reference_fit_PeakingBkg", fitParams, true);
   ...
   w->writeToFile("/home/belle2/amubarak/Ds2D0enue_Analysis/10-Suggestion/Roofit_Suggestion_Chi/Fit_Parameter/rs01_DstarP.root");
   //=========================================================================================================================================================================//
   c->SaveAs("/home/belle2/amubarak/Ds2D0enue_Analysis/10-Suggestion/Roofit_Suggestion_Chi/Fit_Image/rs01_DeltaM_DstarP.png");
   //=========================================================================================================================================================================//
   // Final Fit Status
   //---------------------------
   ...
   int ndof = model.getParameters(data_Dstarplus)->selectByAttrib("Constant", kFALSE)->getSize(); 
