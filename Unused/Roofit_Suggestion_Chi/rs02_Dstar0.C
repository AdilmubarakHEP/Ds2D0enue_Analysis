...
void rs02_Dstar0()
{
   ...
   TH1F* hh = (TH1F*)file->Get("hist_Dstar0");
   //=========================================================================================================================================================================//
   // D a t a:
   // --------------------------------------------------------------
   RooRealVar DeltaM("Ds_diff_D0pi", "Mass Difference", xL, xR);
   RooDataHist data_Dstar0("data_Dstar0", "dataset with mass", DeltaM, hh);
   //=========================================================================================================================================================================//
   ...
   // RooGenericPdf Generic0_Dstar0Bkg("Generic0_Dstar0Bkg","Generic PDF","(Ds_diff_D0pi - 0.13953 > 0 ? ( ((Ds_diff_D0pi-0.13953)^P0_Dstar0Bkg)*exp( B0_Dstar0Bkg*(Ds_diff_D0pi-0.13953))) : 0.0)", RooArgSet( DeltaM, B0_Dstar0Bkg, P0_Dstar0Bkg));
   ...
   // RooGenericPdf Generic1_Dstar0Bkg("Generic1_Dstar0Bkg","Generic PDF","(Ds_diff_D0pi - 0.13953 > 0 ? ( ((Ds_diff_D0pi-0.13953)^P1_Dstar0Bkg)*exp( B1_Dstar0Bkg*(Ds_diff_D0pi-0.13953))) : 0.0)", RooArgSet( DeltaM, B1_Dstar0Bkg, P1_Dstar0Bkg));
   ...
   // RooAddPdf CFBkgModel("CFBkgModel", "b+c", RooArgList(Generic0_Dstar0Bkg,Generic1_Dstar0Bkg), RooArgList(f0_Dstar0Bkg,f1_Dstar0Bkg));

   // // Second Attempt
   // //---------------------
   // // First Gamma Function
   // RooRealVar x00_Dstar0Bkg("x00_Dstar0Bkg","x00_Dstar0Bkg", 1.3907e-01, 0.13, 0.2);
   // RooRealVar a0_Dstar0Bkg("a0_Dstar0Bkg","a0_Dstar0Bkg", 1.1997e+00, 6.0e-01, 12.0e-01);
   // RooRealVar b0_Dstar0Bkg("b0_Dstar0Bkg","b0_Dstar0Bkg", 1.0932e+02, 7.0e+01, 11.0e+01);    
   ...
   // RooRealVar x01_Dstar0Bkg("x01_Dstar0Bkg","x01_Dstar0Bkg", 1.5246e-01, 0.13, 0.2);
   // RooRealVar a1_Dstar0Bkg("a1_Dstar0Bkg","a1_Dstar0Bkg", 6.7523e-01, 5.0e-01, 11.0e-01);
   // RooRealVar b1_Dstar0Bkg("b1_Dstar0Bkg","b1_Dstar0Bkg", 5.1570e+00, 4.0e+00, 8.0e+00);     
   ...
   // RooRealVar x02_Dstar0Bkg("x02_Dstar0Bkg","x02_Dstar0Bkg", 1.5785e-01, 0.13, 0.2);
   // RooRealVar a2_Dstar0Bkg("a2_Dstar0Bkg","a2_Dstar0Bkg", 1.2908e+00, 0.0e+00, 4.0e+00);
   // RooRealVar b2_Dstar0Bkg("b2_Dstar0Bkg","b2_Dstar0Bkg", 3.1227e+01, 2.0e+01, 6.0e+01);     
   ...
   // RooRealVar x03_Dstar0Bkg("x03_Dstar0Bkg","x03_Dstar0Bkg", 1.3021e-01, 0.13, 0.15);
   // RooRealVar a3_Dstar0Bkg("a3_Dstar0Bkg","a3_Dstar0Bkg", 9.9197e+00, 0.0, 10.);
   // RooRealVar b3_Dstar0Bkg("b3_Dstar0Bkg","b3_Dstar0Bkg", 4.9667e+01, 0.0, 50.);     
   ...
   // // Add the components
   // RooRealVar f0_Dstar0Bkg("f0_Dstar0Bkg", "f0", 3.3138e-02, 1.0e-02, 5.0e-02);
   // RooRealVar f1_Dstar0Bkg("f1_Dstar0Bkg", "f1", 6.4157e-01, 4.0e-01, 9.0e-01);
   // RooRealVar f2_Dstar0Bkg("f2_Dstar0Bkg", "f2", 8.9399e-02, 2.0e-02, 9.0e-02);
   // RooRealVar f3_Dstar0Bkg("f3_Dstar0Bkg", "f3", 2.9248e-02, 0, 1);
   ...
   // // Third Attempt
   // //----------------
   // RooRealVar c0("c0", "coefficient #0", -10000, 10000);
   // RooRealVar c1("c1", "coefficient #1", -10000, 10000);
   // RooRealVar c2("c2", "coefficient #2", -10000, 10000);
   // RooRealVar c3("c3", "coefficient #3", -10000, 10000);
   // RooRealVar c4("c4", "coefficient #4", -10000, 10000);
   // // RooRealVar c5("c5", "coefficient #5", -10000, 10000);
   // // RooRealVar c6("c6", "coefficient #6", -10000, 10000);
   // // c0.setConstant(true);
   // // c1.setConstant(true);
   // // c2.setConstant(true);
   // // c2.setConstant(true);
   // // c3.setConstant(true);
   // // c4.setConstant(true);
   // RooPolynomial Dstar0BkgModel("Dstar0BkgModel", "Other", DeltaM, RooArgList(c0,c1,c2,c3,c4));
   ...
   RooRealVar c0("c0", "coefficient #0", -7.8719e+01, -8.0e+01, -3.0e+01);
   RooRealVar c1("c1", "coefficient #1", 1.3065e+03, 0.0e+03, 3.0e+03);
   RooRealVar c2("c2", "coefficient #2", 1.1131e-02, 0.0e+01, 5.0e+01);
   RooRealVar c3("c3", "coefficient #3", -7.1648e+03 -9.0e+03, -5.0e+03);
   RooRealVar c4("c4", "coefficient #4", 9.5703e+03, 6.0e+03, 10.0e+03);
   RooRealVar c5("c5", "coefficient #5", 9.9666e+03, -10000, 10000);
   // RooRealVar c6("c6", "coefficient #6", -10000, 10000);
   c0.setConstant(true);
   c1.setConstant(true);
   c2.setConstant(true);
   ...
   c4.setConstant(true);
   c5.setConstant(true);
   RooPolynomial PolykgModel("PolykgModel", "Other", DeltaM, RooArgList(c0,c1,c2,c3,c4,c5));
   ...
   //-----------------------------------------
   RooRealVar Nsig_Dstar0Bkg("Nsig_Dstar0Bkg", "#Signal events", 1.7823e+05, 0.5e+05, 4.0e+05);
   ...
   RooChi2Var chi2("chi2", "chi2", model, data_Dstar0, DataError(RooAbsData::SumW2));
   //=========================================================================================================================================================================//
   // F i t
   //-------------
   // Perform extended ML fit of data:
   // std::unique_ptr<RooFitResult> r{model.fitTo(data_Dstar0, Save(), PrintLevel(-1))};
   std::unique_ptr<RooFitResult> r{model.chi2FitTo(data_Dstar0, Save(), PrintLevel(-1))};
   //=========================================================================================================================================================================//
   ...
   // model.plotOn(frame, Components(Generic0_Dstar0Bkg),       LineStyle(kDashed), LineColor(46), RooFit::Name("Background"));
   // model.plotOn(frame, Components(Generic1_Dstar0Bkg), LineStyle(kDashed), LineColor(38), RooFit::Name("Signal"));
   model.plotOn(frame, LineColor(kOrange+7), RooFit::Name("model"));
   //=========================================================================================================================================================================//
   ...
   frame->GetYaxis()->SetRangeUser(0,7000);
   ...
   legend->AddEntry("model", "D^{*0} #rightarrow [D^{0} #rightarrow K^{-} #pi^{+}] #gamma/#pi^{0}", "pl" );
   ...
   w->saveSnapshot("reference_fit_Dstar0_Bkg", fitParams, true);
   ...
   w->writeToFile("/home/belle2/amubarak/Ds2D0enue_Analysis/10-Suggestion/Roofit_Suggestion_Chi/Fit_Parameter/rs02_Dstar0.root");
   //=========================================================================================================================================================================//
   c->SaveAs("/home/belle2/amubarak/Ds2D0enue_Analysis/10-Suggestion/Roofit_Suggestion_Chi/Fit_Image/rs02_DeltaM_Dstar0.png");
   //=========================================================================================================================================================================//
   // Final Fit Status
   //---------------------------
   ...
   int ndof = model.getParameters(data_Dstar0)->selectByAttrib("Constant", kFALSE)->getSize(); 
